...
MicroTask Queue:
Promises are executed in this microtask queue. They are executed before the next task is executed. If the task queue is empty and there are pending microtasks, the event loop will execute all of the pending microtasks before moving on to the next task. Micro task queues has high priority than task queues. Promises are executed in this microtask queue. They are executed before the next task is executed. If the task queue is empty and there are pending microtasks, the event loop will execute all of the pending microtasks before moving on to the next task.
